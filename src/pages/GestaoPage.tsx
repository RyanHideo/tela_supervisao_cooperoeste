      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 space-y-1 text-sm">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-[11px]">
                  <div className="rounded-lg border border-emerald-400/50 bg-emerald-500/10 px-2 py-1">
                    <p className="uppercase tracking-[0.2em] text-emerald-300">CCM 1</p>
                    <p className="text-lg font-semibold text-emerald-100">
                      {formatNumber(fatorPotenciaCcm1, 2)}
                    </p>
                  </div>
                  <div className="rounded-lg border border-sky-400/50 bg-sky-500/10 px-2 py-1">
                    <p className="uppercase tracking-[0.2em] text-sky-200">CCM 2</p>
                    <p className="text-lg font-semibold text-sky-100">
                      {formatNumber(fatorPotenciaCcm2, 2)}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <div className={`${panelClass} p-4 lg:col-span-2`}>
            ...
            <div className="mt-6 h-64 rounded-2xl border border-slate-800/60 bg-slate-900/50 p-4">
              <div className="flex h-full items-end gap-3">
                {elevators.map((elevator) => {
                  const pct = Math.max(0, Math.min(100, elevator.value ?? 0));
                  return (
                    <div key={elevator.id} className="flex flex-1 flex-col items-center justify-end gap-2 text-xs">
                      <div className="flex h-full w-full items-end justify-center">
                        <div
                          className="w-6 rounded-t-xl bg-gradient-to-t from-indigo-500 via-sky-500 to-emerald-400 shadow-lg shadow-emerald-500/20"
                          style={{ height: `${pct}%` }}
                        />
                      </div>
                      <span className="font-semibold text-slate-200">{pct ? `${pct.toFixed(0)}%` : "--"}</span>
