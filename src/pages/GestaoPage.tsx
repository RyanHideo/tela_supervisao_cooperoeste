...
export function GestaoPage() {
  ...
  const fatorPotenciaCcm1 = getNumber("CCM1_FATOR_POTENCIA");
  const fatorPotenciaCcm2 = getNumber("CCM2_FATOR_POTENCIA");
  const fatorPotenciaMedio = avg([fatorPotenciaCcm1, fatorPotenciaCcm2]);
    ...
    <div className={rootClass}>
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6">
        <section className={`grid gap-4 lg:grid-cols-12 ${accentGradient} p-4 ${panelClass}`}>
          ...
          <div className="lg:col-span-3 flex flex-col gap-4 rounded-2xl border border-sky-500/30 bg-sky-500/10 p-4">
            ...
            <div className="flex items-center gap-4">
              <div
                ...
                style={{
                  background: `conic-gradient(#22c55e ${Math.min(100, Math.max(0, eficiencia ?? 0)) * 3.6}deg, ${
                    isDark ? "#0f172a" : "#e2e8f0"
                  } 0deg)`,
                }}
              >
                <div className={`absolute inset-3 rounded-full ${isDark ? "bg-slate-900" : "bg-white"}`} />
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                  <span className="text-xs uppercase tracking-wide text-slate-400">FP</span>
                  <span className="text-3xl font-black text-emerald-100">
                    {eficiencia !== undefined ? `${eficiencia.toFixed(0)}%` : "--"}
                  </span>
                </div>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 text-sm">
              {[{ label: "CCM 1", value: fatorPotenciaCcm1, tone: "emerald" }, { label: "CCM 2", value: fatorPotenciaCcm2, tone: "sky" }].map((item) => (
                <div
                  key={item.label}
                ...
                >
                  <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.2em]">
                    <span className={item.tone === "emerald" ? "text-emerald-200" : "text-sky-200"}>{item.label}</span>
                    <span className="text-slate-400">FP</span>
                  </div>
                  <p className="text-2xl font-black text-slate-50">{formatNumber(item.value, 2)}</p>
                  <div className="mt-2 h-2 rounded-full bg-slate-900/60">
                    <div
                      ...
                      style={{ width: barWidth(item.value, 1) }}
                    />
                  </div>
                </div>
              ))}
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <div className={`${panelClass} p-4 lg:col-span-2`}>
            ...
            <div className="mt-6 h-72 rounded-2xl border border-slate-800/60 bg-slate-900/50 p-4">
              <div className="relative h-full">
                <div className="absolute inset-0 px-8 py-6">
                  <div className="flex h-full flex-col justify-between">
                    {[100, 75, 50, 25, 0].map((tick) => (
                      <div key={tick} className="flex items-center gap-2 text-[11px] text-slate-500">
                ...
                <div className="relative flex h-full items-end gap-3 px-2 pb-4 pl-10">
                  {elevators.map((elevator) => {
                    const pct = Math.max(0, Math.min(100, elevator.value ?? 0));
                    return (
                      <div key={elevator.id} className="relative flex flex-1 flex-col items-center justify-end gap-2 text-xs">
                        <div className="flex h-full w-full items-end justify-center">
                          <div
                            className="w-7 rounded-t-xl bg-gradient-to-t from-indigo-600 via-sky-500 to-emerald-400 shadow-lg shadow-emerald-500/20"
                            style={{ height: `${pct}%` }}
                          />
                        </div>
                        <span className="absolute -top-6 rounded-full bg-slate-800 px-2 py-1 text-[11px] font-semibold text-slate-200 shadow">{pct ? `${pct.toFixed(0)}%` : "--"}</span>
                        <span className="text-[11px] uppercase tracking-wide text-slate-500">
                          Elev. {String(elevator.id).padStart(2, "0")}
                        </span>
                      </div>
                    );
                  })}
